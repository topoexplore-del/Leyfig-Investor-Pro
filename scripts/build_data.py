...
def detect_patterns(hist, lookback=140):
    """Devuelve hasta 3 patrones: [{name,tipo,icon,conf,obj,esc,lines}].
    - obj: objetivo clásico por "measured move" (altura del patrón proyectada).
    - esc: escenario en una frase, con niveles.
    - lines: segmentos [[x1,y1,x2,y2],...] en coordenadas de la ventana
      (x = índice de barra dentro de las últimas `lookback` velas) para
      dibujar el patrón sobre el gráfico.
    Honestidad: el objetivo es la proyección técnica clásica, no una promesa;
    la confirmación sigue siendo la ruptura con volumen."""
    if len(hist) < 60:
        return []
    ...
    # ── Dobles y triples techos/suelos (objetivo = altura desde el cuello) ──
    if len(ph) >= 2:
        (i1, v1), (i2, v2) = ph[-2], ph[-1]
        if i2 - i1 >= 8 and abs(v2 - v1) / max(v1, v2) <= 0.025:
            valley = float(c.iloc[i1:i2 + 1].min())
            vx = i1 + int(c.iloc[i1:i2 + 1].values.argmin())
            if valley <= min(v1, v2) * 0.96 and close < max(v1, v2) * 0.995:
                ...
                triple = len(ph) >= 3 and abs(ph[-3][1] - v1) / v1 <= 0.03
                if triple:
    ...
    if len(pl) >= 2:
        ...
        if i2 - i1 >= 8 and abs(v2 - v1) / max(v1, v2) <= 0.025:
            ...
            if peak >= max(v1, v2) * 1.04 and close > min(v1, v2) * 1.005:
                ...
                triple = len(pl) >= 3 and abs(pl[-3][1] - v1) / v1 <= 0.03
                if triple:
    ...
    if sh is not None and sl is not None:
        ...
        if h_flat and l_up:
            ...
            add("Triángulo ascendente", "bullish", 68, obj,
                f"Alcista: romper la resistencia ${res:.2f} proyecta ≈ ${obj:.2f} ({pct(obj):+.1f}%)", two)
        elif l_flat and h_dn:
            sup = ml * (n - 1) + bl_
            obj = sup - height
            add("Triángulo descendente", "bearish", 68, obj,
                f"Bajista: perder el soporte ${sup:.2f} proyecta ≈ ${obj:.2f} ({pct(obj):+.1f}%)", two)
        elif h_dn and l_up:
            ...
            if sl > sh * 1.5:
                ...
                add("Cuña ascendente", "bearish", 62, obj,
                    f"Bajista al romper abajo: proyección ≈ ${obj:.2f} ({pct(obj):+.1f}%)", two)
            elif abs(sh - sl) <= max(0.05, 0.35 * abs(sh)):
                obj = close + height
                add("Canal alcista", "bullish", 65, obj,
                    f"Tendencia alcista vigente; continuar el canal apunta ≈ ${obj:.2f} ({pct(obj):+.1f}%). Perder la base lo anula", two)
        elif h_dn and l_dn:
            if abs(sh) > abs(sl) * 1.5:
                obj = close + height
                add("Cuña descendente", "bullish", 62, obj,
                    f"Alcista al romper arriba: proyección ≈ ${obj:.2f} ({pct(obj):+.1f}%)", two)
            elif abs(sh - sl) <= max(0.05, 0.35 * abs(sh)):
                obj = close - height
                add("Canal bajista", "bearish", 65, obj,
                    f"Tendencia bajista vigente; continuar el canal apunta ≈ ${obj:.2f} ({pct(obj):+.1f}%). Romper el techo lo anula", two)
    ...
    if len(ph) >= 3:
        ...
        if (max(vals) - min(vals)) / np.mean(vals) <= 0.015:
            lvl = float(np.mean(vals))
            add("Resistencia horizontal", "neutral", 66, None,
                f"Resistencia probada 3+ veces en ${lvl:.2f}: romperla con volumen abre continuación alcista",
    ...
    # ── Banderas y banderines (objetivo = mástil proyectado) ──
